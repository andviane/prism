...
public class Prism extends View {
  ...
  public double a2(double a1) {
    return Math.asin(n * Math.sin(aa - Math.asin(Math.sin(a1) / n)));
  }
  ...
  public void setRayEntry(double rayEntry) {
  ...
  private void resizePrism(MotionEvent event) {
    ...
    double w = (int) Math.abs(p1.x - p2.x);

    double a = topAngle;

    if (primaryWidth == 0 || primaryA == 0) {
      primaryWidth = w;
      primaryA = a;
      primaryRay = rayEntry;
      return;
    ...
    int an = (int) (primaryA * w / primaryWidth);
    if (an > 5 && an < 70) {
      setTopAngle(an);
      setRayEntry(primaryRay + (an - primaryA) / 2 );
    }
  }
}
